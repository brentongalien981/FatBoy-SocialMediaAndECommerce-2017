<!--<link href="--><?php //echo LOCAL . "/public/_styles/notifications/rateable_item/index.css"; ?><!--" rel="stylesheet" type="text/css">-->


<?php // require_once(PUBLIC_PATH . "/__view/notifications/rateable_item/create.php"); ?>
<?php  require_once(PUBLIC_PATH . "/__view/notifications/rateable_item/read.php"); ?>
<?php // require_once(PUBLIC_PATH . "/__view/notifications/rateable_item/update.php");   ?>
<?php // require_once(PUBLIC_PATH . "/__view/notifications/rateable_item/delete.php");   ?>


<script src="<?php echo LOCAL . "/public/_scripts/notifications/rateable_item/instance_vars.js"; ?>"></script>
<script src="<?php echo LOCAL . "/public/_scripts/notifications/rateable_item/general_functions.js"; ?>"></script>
<!--<script src="--><?php //echo LOCAL . "/public/_scripts/notifications/rateable_item/create.js"; ?><!--"></script>-->
<script src="<?php echo LOCAL . "/public/_scripts/notifications/rateable_item/read.js"; ?>"></script>
<!--<script src="--><?php //echo LOCAL . "/public/_scripts/notifications/rateable_item/update.js"; ?><!--"></script>-->
<!--<script src="--><?php //echo LOCAL . "/public/_scripts/notifications/rateable_item/delete.js"; ?><!--"></script>-->
<script src="<?php echo LOCAL . "/public/_scripts/notifications/rateable_item/fetch.js"; ?>"></script>
<script src="<?php echo LOCAL . "/public/_scripts/notifications/rateable_item/NotificationRateableItem.js"; ?>"></script>
<script src="<?php echo LOCAL . "/public/_scripts/notifications/rateable_item/tasks.js"; ?>"></script>



<?php // TODO:SECTION: Supporting scripts when the link "accept" is clicked. ?>